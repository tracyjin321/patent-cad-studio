...
def bearing(p: dict[str, Any]) -> str:
    ...
    dimension = _dimension(245, 545, 655, 545, "⌀{}".format(p["outer_diameter"]))
    return _svg(f'<circle class="o" cx="450" cy="310" r="205"/><circle class="o" cx="450" cy="310" r="165"/>{balls}<circle class="o" cx="450" cy="310" r="92"/><path class="h" d="M180 310H720M450 55V565"/>{dimension}', "滚动轴承")
...
def flange(p: dict[str, Any]) -> str:
    ...
    dimension = _dimension(240, 550, 660, 550, "⌀{}".format(p["outer_diameter"]))
    return _svg(f'<circle class="o" cx="450" cy="310" r="210"/><circle class="h" cx="450" cy="310" r="145"/>{holes}<circle class="o" cx="450" cy="310" r="84"/><path class="h" d="M190 310H710M450 50V570"/>{dimension}', "圆形法兰")


def valve(p: dict[str, Any]) -> str:
    dimension = _dimension(130, 500, 770, 500, str(p["body_length"]))
    c = f'<path class="o" d="M130 260H275L330 205H570L625 260H770V390H625L570 445H330L275 390H130Z"/><path class="o" d="M330 205Q450 350 570 205M330 445Q450 300 570 445"/><path class="o" d="M420 205V120H480V205M365 120H535M390 90H510M450 90V45"/><circle class="o" cx="450" cy="67" r="72"/><path class="h" d="M450 170V470"/>{dimension}'
    return _svg(c, "截止阀")
...
def gear(p: dict[str, Any]) -> str:
    ...
    dimension = _dimension(230, 560, 670, 560, "m={} · z={}".format(p["module"], p["teeth"]))
    c = f'<polygon class="o" points="{point_text}"/><circle class="o" cx="450" cy="310" r="165"/><circle class="o" cx="450" cy="310" r="65"/><rect class="o" x="440" y="245" width="20" height="50"/><path class="h" d="M180 310H720M450 45V575"/>{dimension}'
...
def screw(p: dict[str, Any]) -> str:
    ...
    wave_text = " ".join(waves)
    dimension = _dimension(150, 455, 750, 455, "{} · 导程 {}".format(p["length"], p["lead"]))
    c = f'<path class="o" d="M90 275H150V240H750V275H810V345H750V380H150V345H90Z"/><path class="o" d="{wave_text}"/><path class="h" d="M70 310H830"/>{dimension}'
    return _svg(c, "精密丝杠")
...
def coupling(p: dict[str, Any]) -> str:
    ...
    dimension = _dimension(280, 500, 620, 500, str(p["length"]))
    c = f'<path class="o" d="M110 270H280V175H390V220H510V175H620V270H790V350H620V445H510V400H390V445H280V350H110Z"/>{circles}<path class="h" d="M70 310H830"/>{dimension}'
    return _svg(c, "法兰联轴器")


def seal(p: dict[str, Any]) -> str:
    dimension = _dimension(225, 500, 675, 500, "⌀{} / ⌀{}".format(p["outer_diameter"], p["inner_diameter"]))
    c = f'<path class="o" d="M225 175H675V445H225Z"/><path class="o" d="M285 230H615V390H285Z"/><path class="o" d="M285 230L375 310L285 390M615 230L525 310L615 390"/><circle class="o" cx="450" cy="310" r="66"/><path class="h" d="M180 310H720M450 120V500"/>{dimension}'
    return _svg(c, "唇形密封件")
