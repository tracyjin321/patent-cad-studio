...
- 图元推荐接口现同时返回目录匹配、缺失部件和自然语言装配关系；配置 Moonshot 时使用严格结构化输出，失败时保留确定性目录匹配。
- 单个未命中、且可映射到现有参数化生成器的图元，会进入“大模型参数提取 → ComponentSpec/YAML → STEP/预览”主流程。
- “已有图元 + 缺失图元”的混合装配、多个缺失图元或未知空间拓扑会返回 `manual_rules_required`，避免在缺少合法端口和约束求解器时静默猜测。
- 装配 3D 预览按实例输出 mesh，不同图元使用不同颜色；左侧悬浮图例展示图元和实例数量，并支持悬停聚焦。
